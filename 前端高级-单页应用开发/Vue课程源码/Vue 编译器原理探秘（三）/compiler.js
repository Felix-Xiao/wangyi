(function(root) {
	...
	function parse(template, options) {
		...
		var stack = [];
		parseHTML(template, {
			start: function(tag, attrs, unary) { //当前元素span
				var element = createASTElement(tag, attrs, currentParent); //AST对象
				...
				if (!root) { //解析根元素 root === undefined
					root = element;
					checkRootConstraints(root);
				} else if (!stack.length) { // 解析的不是根元素   
				...
				if (currentParent && !element.forbidden) {
					if (element.elseif || element.else) {   //if  else if
						...
						currentParent.children.push(element);  //span   子父级关系 完成的
						element.parent = currentParent;
					}
				}
				if (!unary) {  //true
					currentParent = element;
					stack.push(element);    //[{div...},{span...}]
				} else {
	...
	function parseHTML(html, options) {
		...
		while (html) {
			if (!lastTag || !isPlainTextElement(lastTag)) {
				...
				if (textEnd === 0) {
					...
					if (endTagMatch) {
						var curIndex = index; //curIndex 结束标签的字符流起始位置
		...
		/*
		tagName 标签名 start 字符流起始位置  end 结束位置
		*/
		function parseEndTag(tagName, start, end) {
			...
			if (tagName) {
				...
				for (pos = stack.length - 1; pos >= 0; pos--) { //1   h1
					if (stack[pos].lowerCasedTag === lowerCasedTagName) { //span span
						break
					}
				}
			} else {
			...
			if (pos >= 0) {
				...
				for (var i = stack.length - 1; i >= pos; i--) {
					if (i > pos || !tagName && //有非一元标签未闭合
						options.warn
					) {
						options.warn( //错误信息 警告信息 提取的方法
				...
				stack.length = pos; //重置下  一堆标签检测完成  
				lastTag = pos && stack[pos - 1].tag;
			} else if (lowerCasedTagName === 'br') { // -1
				if (options.start) {
					options.start(tagName, [], true, start, end);
				}
			} else if (lowerCasedTagName === 'p') { //   -1  在AST 记录说明p | br 是根元素下的子节点
				if (options.start) {
